...
const useChangePasswordWithToken = () => {
        ...
        const check = useAction(api.users.GetUserByTokenAction);
        ...
        const CheckUser = async (token:string,Newpassword:string) =>{
                ...
                console.log("User found by token:", Newpassword);
...
>
    ...
    <div class="footer">
      &copy; 2025 ShopCheap. All rights reserved.
                ...
                `
                await UpdateUser({ User: UserToUpdate }).then(async (res)=>{
                        if(!res?.succes){
                                return { success: false, message: res?.message ,  status: 400 };
                        }
                        const resp = await sendEmail(user?.email||"","Password Changed",html)
